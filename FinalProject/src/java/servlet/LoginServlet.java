...
public class LoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try{        
            ...
            ResultSet rs=getUser.query(sql);

            if(rs!=null){ //rs is not null, which means account is in the database
                session.setAttribute("userId",userName);

                System.out.println(response.isCommitted());
                response.sendRedirect("index.htm") ; //send to home page
            }
            else{ //account is not in the databse, or incorrect user ID or password
